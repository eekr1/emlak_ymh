...
export function inferHandoffFromText(text) {
    ...
    // ✅ CATEGORY sinyali (Emlak)
    const lower = text.toLowerCase();
    let category = "diger";
    if (/satılık|satın|almak|ev arıyorum|daire arıyorum/.test(lower)) category = "satılık";
    else if (/kiralık|kiralamak|tutmak/.test(lower)) category = "kiralık";
    else if (/arsa|tarla|arazi|parsel|imar/.test(lower)) category = "arsa";
    else if (/dükkan|ofis|ticari|depo|plaza|devren/.test(lower)) category = "ticari";
    else if (/danışmanlık|ekspertiz|değerleme/.test(lower)) category = "danismanlik";

    const urgency = /acil|bugün|yarın|hemen|taşınmam lazım/i.test(text) ? "acil" : "normal";
